  ...
  _updateDecorations: function _updateDecorations(duration) {
    ...
    if (showxAxisGroups) {
      ...
      xAxisGroups.each(function (d, i) {
        ...
        var text = view.select("text.vzb-bc-x-axis-group-text").transition().duration(duration || 0).style("text-anchor", isFirst ? "end" : isLast ? "start" : "middle").attr("y", calcs.textHeight).attr("x", x);

        if (calcs.marginX_px < 0 && !isFirst && !isLast) text.text(i + 1);

        view.select("text.vzb-bc-x-axis-group-line").classed("vzb-invisible", isLast).transition().duration(duration || 0).attr("y", calcs.textHeight * 0.9).attr("x", calcs.boundaryMaxX_px);
      });
    }
